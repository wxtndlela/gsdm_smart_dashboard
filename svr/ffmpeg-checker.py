...
def is_service_running(port):
    ...
    result = sock.connect_ex(('192.168.15.187', port))
    sock.close()
    return result == 0

def start_service():
    # Rerun the service using the specified command
    #subprocess.Popen('MAP-SERVICE.bat', shell=True)
    vbscript_file = "MAP-SERVICE.vbs"
    subprocess.Popen('cscript.exe "{}"'.format(vbscript_file), shell=True)
...
port_number = 3000
while True:
    if is_service_running(port_number):
        print("The service is running.")
    else:
        print("The service is not running. Starting the service...")
